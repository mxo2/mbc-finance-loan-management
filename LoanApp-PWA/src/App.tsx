...
// Loan Card Component for displaying individual loans
function LoanCard({ loan, isRunning }: { loan: any; isRunning: boolean }) {
  // Calculate EMI statistics for running loans
  const calculateLoanStats = (loan: any) => {
    if (!isRunning || loan.status !== 'Approved') {
      return null
    }
    
    const totalLoanValue = parseFloat(loan.amount || 0)
    const paidAmount = parseFloat(loan.paid_amount || 0)
    const pendingAmount = parseFloat(loan.pending_amount || 0)
    
    return {
      totalLoanValue,
      totalEMIPaid: paidAmount,
  ...
  return (
    <div className={`bg-white rounded-xl shadow-sm border hover:shadow-lg transition-all duration-300 p-6 ${
      isRunning ? 'border-green-200 hover:border-green-300' : 'border-yellow-200 hover:border-yellow-300'
    }`}>
      {/* Header */}
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-lg font-semibold text-gray-900">{loan.type || 'Personal Loan'}</h3>
      ...
      <div className="space-y-3 mb-4">
        <div className="flex justify-between">
          ...
          <span className="font-semibold text-gray-900">₹{(loan.amount || 0).toLocaleString()}</span>
        </div>
        
        {isRunning && (
          <div className="flex justify-between">
            <span className="text-gray-600">Outstanding:</span>
            <span className="font-semibold text-red-600">₹{(loan.pending_amount || 0).toLocaleString()}</span>
          </div>
        )}
        ...
        <div className="flex justify-between">
          ...
          <span className="font-medium">{loan.terms || 'N/A'} {loan.term_period || 'months'}</span>
        </div>
        
        <div className="flex justify-between">
          <span className="text-gray-600">Interest Rate:</span>
          <span className="font-medium">{loan.interest_rate || 'N/A'}% per annum</span>
        </div>
        
        {isRunning && loan.start_date && (
          <div className="flex justify-between">
      ...
      {/* EMI Progress for Running Loans */}
      {stats && (
        <div className="mt-4 pt-4 border-t border-gray-100">
          <div className="text-sm font-medium text-gray-900 mb-3">Payment Progress</div>
            ...
            <div className="mt-3">
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div 
                  ...
                  style={{width: `${stats.totalLoanValue > 0 ? ((stats.totalEMIPaid / stats.totalLoanValue) * 100).toFixed(1) : 0}%`}}
                ></div>
              </div>
              <div className="text-xs text-gray-500 mt-1 text-center">
                {stats.totalLoanValue > 0 ? ((stats.totalEMIPaid / stats.totalLoanValue) * 100).toFixed(1) : 0}% completed
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="mt-6 pt-4 border-t border-gray-200">
        {isRunning ? (
          ...
          <div className="text-center">
            <span className="inline-flex items-center px-4 py-2 bg-yellow-50 text-yellow-800 text-sm font-medium rounded-lg">
    ...
    const fetchLoans = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (data.success) {
            // Show ALL loans - both running and applied
            setLoans(data.loans || [])
          } else {
            setLoans([])
          }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Loan Overview</h1>
          <p className="text-gray-600">Manage all your loans in one place</p>
        </div>

        {/* Separate loans into categories */}
        {(() => {
          const runningLoans = loans.filter(loan => 
            loan.status === 'Approved' || loan.status === 'approved' || loan.status === 'Active'
          )
          const appliedLoans = loans.filter(loan => 
            loan.status === 'Pending' || loan.status === 'pending' || loan.status === 'Under Review'
          )
            ...
            <div className="space-y-12">
                ...
                <div>
                  <div className="flex items-center mb-6">
                    <div className="flex items-center">
                      ...
                      <h2 className="text-2xl font-bold text-gray-900">Running Loans</h2>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {runningLoans.map((loan, index) => (
                      <LoanCard key={loan.id || index} loan={loan} isRunning={true} />
                    ))}
                  </div>
                </div>
              )}
                  ...
                  <div className="flex items-center mb-6">
                    <div className="flex items-center">
                      ...
                      <span className="ml-3 bg-yellow-100 text-yellow-800 text-sm font-medium px-3 py-1 rounded-full">
                        {appliedLoans.length} Pending
                      </span>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {appliedLoans.map((loan, index) => (
                      <LoanCard key={loan.id || index} loan={loan} isRunning={false} />
                    ))}
                  </div>
                </div>
              )}
              ...
              {loans.length === 0 && (
                <div className="text-center py-16">
                  ...
                  <p className="text-gray-600 mb-6">You don't have any loans yet. Apply for a new loan to get started.</p>
                  <a
                    href="/pwa/loan-application"
...
function Dashboard() {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true)
        const response = await fetch('/api/pwa/dashboard', {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            'Accept': 'application/json',
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
            ...
            <a href="/pwa/pay-emi" className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow group block">
            ...
            <a href="/pwa/schedule" className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow group block">
            ...
            <a href="/pwa/transactions" className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow group block">
            ...
            <a href="/pwa/emi-calculator" className="bg-white rounded-xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition-shadow group block">
...
function App() {
  return (
    <Routes>
      ...
      <Route path="/pwa/schedule" element={<SchedulePage />} />
      <Route path="/pwa/transactions" element={<TransactionsPage />} />
      <Route path="/pwa/emi-calculator" element={<EMICalculatorPage />} />
      <Route path="/pay-emi" element={<PayEMI />} />
      <Route path="/pwa/pay-emi" element={<PayEMI />} />
      <Route path="/upcoming-installments" element={<PayEMI />} />
      <Route path="/pwa/upcoming-installments" element={<PayEMI />} />
